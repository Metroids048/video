...
def _run(ep_dir: Path, *, timeline: dict[str, object] | None = None, metadata: dict[str, object] | None = None, black: list[dict[str, float]] | None = None, silence: list[dict[str, float]] | None = None, peak: float | None = -6.0, subtitles: dict[str, object] | None = None) -> dict:
    ...
    with (
        ...
        return run_qa(ep_dir, "EP-QA-TEST", force=True)
...
def test_black_frame_over_one_second_fails_qa(tmp_path: Path) -> None:
    report = _run(_episode(tmp_path), black=[{"start": 1.0, "end": 2.2, "duration": 1.2}])
...
def test_planned_audio_with_long_silence_fails_qa(tmp_path: Path) -> None:
    ...
    report = _run(_episode(tmp_path), timeline=timeline, silence=silence)
    assert report["passed"] is False


def test_low_resolution_and_clipping_fail_qa(tmp_path: Path) -> None:
    report = _run(_episode(tmp_path), metadata=_metadata(width=720, height=1280), peak=-0.2)
...
def test_subtitle_overflow_fails_but_placeholder_is_warning(tmp_path: Path) -> None:
    ...
    report = _run(_episode(tmp_path), timeline=timeline, subtitles=subtitles)
    assert report["passed"] is False
...
def test_placeholder_warning_does_not_block_qa(tmp_path: Path) -> None:
    timeline = {"errors": [], "warnings": [], "placeholder_count": 3, "planned_audio": False, "total_duration": 9.0}
    report = _run(_episode(tmp_path), timeline=timeline)
    assert report["passed"] is True
