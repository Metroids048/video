...
本地参考片会先进入 `analyze-reference`，再由 `video-shotcraft` 只提供镜头语法、节奏和声音设计参考；主渲染仍走 AVS 的 `timeline.json`、FFmpeg 和 HyperFrames。改编时必须替换文案、配音、素材、案例、数据、观点、标题和封面。
...
缺少素材、Provider 或授权时，我会停在明确的 `WAITING_FOR_INPUT` / `BLOCKED`，列出缺口和替代方案；不会把“做出来了”写成“能发布”或“能赚钱”。
