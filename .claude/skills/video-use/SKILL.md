...
- **Free default:** no ElevenLabs key needed. Transcription uses project `scripts/free_providers/whisper_transcribe.py` (`faster-whisper`) via `helpers/transcribe.py --backend auto`.
- Optional: `ELEVENLABS_API_KEY` only if you want Scribe quality upgrade.
- `ffmpeg` + `ffprobe` on PATH.
- Python deps installed (`uv sync` or `pip install -e .` inside the repo) **plus** `faster-whisper`.
